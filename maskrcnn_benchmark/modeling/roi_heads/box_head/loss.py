...
class FastRCNNVGLossComputation(object):
    """
    Computes the loss for Faster R-CNN.
    Also supports FPN
    """
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            attr_labels_per_image = attr_labels_per_image.to(dtype=torch.float32)
            ...
            attr_labels_per_image[bg_inds, :] = 0

            # Label ignore proposals (between low and high thresholds)
            ignore_inds = matched_idxs == Matcher.BETWEEN_THRESHOLDS
            labels_per_image[ignore_inds] = -1  # -1 is ignored by sampler
            attr_labels_per_image[ignore_inds, :] = 0
        ...
        return labels, regression_targets, attr_labels
    ...
    def subsample(self, proposals, targets):
        ...
        proposals = list(proposals)
        # add corresponding label and regression_targets information to the bounding boxes
        for labels_per_image, regression_targets_per_image, attr_labels_per_image, proposals_per_image in zip(
            labels, regression_targets, attr_labels, proposals
        ):
            proposals_per_image.add_field("labels", labels_per_image)
            proposals_per_image.add_field(
                "regression_targets", regression_targets_per_image
    ...
    def __call__(self, class_logits, box_regression, attr_logits):
        ...
        attr_labels = cat([proposal.get_field("attr_labels") for proposal in proposals], dim=0)
        ...
        eps = 1e-8
        attr_pos_labels = attr_labels[sampled_pos_inds_subset, :]
        pos_num = attr_pos_labels.sum(dim=1, keepdim=True)
        attr_softmax = F.log_softmax(attr_logits[sampled_pos_inds_subset, :], dim=1)

        attr_loss = -0.5 * (attr_pos_labels * attr_softmax)
        attr_loss = attr_loss.sum(dim=1, keepdim=True)

        attr_loss = attr_loss / (pos_num + eps)
        attr_loss = attr_loss.mean()

        return classification_loss, box_loss, attr_loss

def make_roi_vg_box_loss_evaluator(cfg):
    matcher = Matcher(
        cfg.MODEL.ROI_HEADS.FG_IOU_THRESHOLD,
        cfg.MODEL.ROI_HEADS.BG_IOU_THRESHOLD,
        allow_low_quality_matches=True,
    ...
    loss_evaluator = FastRCNNVGLossComputation(
        matcher, 
        fg_bg_sampler, 
        box_coder, 
        cls_agnostic_bbox_reg
